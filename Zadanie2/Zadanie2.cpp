...
void nearest_points(std::vector<point>& all_points) {
	//zmienne ddo zapisania punktow
	point p1 = { 0,0 };
	point p2 = { 0,0 };

	//zmienne pomocniczne
	float x = all_points[1].x - all_points[0].x;
	float y = all_points[1].y - all_points[0].y;
	float min_width = sqrt((x*x) + (y*y));

	//przeszukujemy wszystkie punkty (kazdy z kazdym), pomijajac te juz sprawdzone
	for (int i = 0; i < all_points.size(); i++) {
		for (int j = i + 1; j < all_points.size(); j++) {
			float d = sqrt(pow((all_points[j].x - all_points[i].x), 2) + pow((all_points[j].y - all_points[i].y), 2));
			if (d < min_width) {
				min_width = d;
				p1.x = all_points[i].x;
				p1.y = all_points[i].y;
	...
	//wypisanie wynikow
	std::cout << "\nd= " << min_width << ", (" << p1.x << ", " << p1.y << "), (" << p2.x << ", " << p2.y << ")";
}
